...
class Player {
  ...
  // ---- The painted avatar as the walking top-down model (owner art) ----
  // Front art faces the camera (moving down/idle), back art shows when
  // walking away, and left/right use the front art mirrored with a lean and
  // a slight shear (a cheap 3/4 turn). The image is drawn in THREE LAYERED
...
class Enemy {
  constructor(type, x, y, opts = {}) {
    ...
    // FORGOTTEN CRYPT tiers multiply HEALTH ONLY (owner rule): ×1.5 per tier.
    const cryptHp = (typeof Items !== 'undefined' && Items.cryptTier) ? Math.pow(1.5, Items.cryptTier()) : 1;
    this.maxHp = Math.round(t.hp * scale * cryptHp);
    this.hp = this.maxHp;
